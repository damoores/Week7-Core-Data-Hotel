...
+ (NSArray *)availableRooms:(NSDate *)endDate startDate:(NSDate *)startDate
{
    ...
    NSError *fetchError;
    NSArray *results = [[[CoreDataStack shared] managedObjectContext] executeFetchRequest:request error:&fetchError];
    ...
    checkResults = [[[CoreDataStack shared] managedObjectContext] executeFetchRequest:checkRequest error:&fetchError];
    if (fetchError) {
